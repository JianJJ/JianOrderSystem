...
//---------------------- 0. 載入需要的資料、設定變數 -------------------------------
    ...
    var myJSONnum = window.localStorage.getItem("InOrderNumber");
    var inOrderNum = JSON.parse(myJSONnum);          // 載入查詢的訂單編號
    var DNumber = 0;                                 // 初始化訂單列

//---------------------- 1. 找出此訂單訂單資料 ------------------------------------
    var SelOrderobj ={};
    for(var i=0;i<OrderArray.length;i++){
        if(OrderArray[i].OrderID == inOrderNum){
            SelOrderobj = OrderArray[i];
            break;
        }
    }
//---------------------- 2. 代入編號,供應商 ---------------------------------------
    $('#OrderID').prop('value',`${SelOrderobj.OrderID}`);
    $('#VendorName').prop('value',`${SelOrderobj.VendorName}`);
    $('#Indate').prop('value',`${SelOrderobj.Indate}`);
...
//---------------------- 3.1 創建商品列--------------------------------------------
    function InsertIn() {                             // 增加一列商品欄位
        ...
        var col = `<tr id=tr${DNumber}>` +            // 給這列(tr)一個id
            ...
            `<td><select id="sel${DNumber}" class="InOinput"></select></td>` +
            `<td id="td2${DNumber}"></td>` +          // '#2DNumber'名稱
            `<td id="td3${DNumber}"></td>` +          // '#3DNumber'規格
            `<td id="td4${DNumber}">` +               // '#4DNumber'單價
                `<input id="i4${DNumber}" value="0" class="InOinput"></td>` +
            `<td id="td5${DNumber}">` +               // '#5DNumber'數量
                `<input id="i5${DNumber}" value="1" class="InOinput"></td>` +
            `<td id="td6${DNumber}">` +               // '#6DNumber'折數
                `<input id="i6${DNumber}" value="1.00" class="InOinput"></td>` +
...
//---------------------- 7. 代入查詢訂單的資料 ------------ --------------------------
    function getValue(DN){
        ...
        if(selnum){                                     // 這裡判斷是不是修改新增的資料
            ...
            for(var i=0;i<ProdaArray.length;i++){       // 帶入的時候 先把訂單的庫存'扣回去'
                if(selnum == ProdaArray[i].ProductNameID){// 更新資料後,儲存時再'加回去'
...
//---------------------- 8. "修改"按鍵,取消input的readony ----------------------------
    $('#reInput').on('click',function(){
        ...
        $('#OrderID,#VendorName,#SumCount').prop('readOnly',true);
...
//---------------------- 8-1. "刪除"的按鍵, 設定刪除訂單的狀態為"NoShow"-----------------
    $('#DelNO').on('click',function(){                   // 不記錄任何修改,新增狀態後
        ...
        if(r){
            for(var i=0;i<OrderArray.length;i++){        // 直接回存資料
                if(OrderArray[i].OrderID == inOrderNum){
                    OrderArray[i]["State"] = "NoShow";
                    break;
                }
            }
...
//---------------------- 8-2. "儲存"的按鍵,儲存資料------------------------------------
    $('#setOver').on('click', function(){
        SelOrderobj = {};
        for(var i=0;i<$('.InOinput').length;i++){
            SelOrderobj[$('.InOinput')[i].id] = $('.InOinput')[i].value;
        }
        for(var i=0;i<OrderArray.length;i++){
            if(parseInt(SelOrderobj.OrderID)==parseInt(OrderArray[i].OrderID)){
                OrderArray.splice(i,1,SelOrderobj);
                break;
            }
        }
        saveLS();                                         // p.9 儲存LocalStorage
