...
//---------------------- 1. 帶入表格抬頭 ------------------------------------------
    $('h3').text(`${inquireObj.startDate} ~ ${inquireObj.endDate} `
                                        +`${inquireObj.CustomerName}`);
//---------------------- 2. 找出該客戶的訂單資料 -----------------------------------
    var sunTotal = 0;                                                // 判斷
    for(var i=0;i<outOrderArray.length;i++){                         // 1. 查詢名稱一樣的訂單
        if(inquireObj.CustomerName == outOrderArray[i].CustomerName  // 2. 不是被刪除的
            && outOrderArray[i].State == 'Show'                      // 3. 日期區間內
            && parseInt(outOrderArray[i].OutDate+0) >= parseInt(inquireObj.startDate+0)
            && parseInt(outOrderArray[i].OutDate+0) <= parseInt(inquireObj.endDate+0)){
                createTable(i,Object.keys(outOrderArray[i]).length); // 計算該筆訂單的資料長度
                $("tr:last-child>td").css('border-bottom','1px solid #9a484c'); // 畫線區分訂單
                sunTotal = sunTotal + parseFloat(outOrderArray[i].SumCount);  // 計算總和
        }
    }
    $('#sunTotal').prop('value',sunTotal);                           // 顯示總和
//---------------------- 3. 創建各個客戶表格,找出商品名稱,計算小計 --------------------
    function createTable(i, len){
        for(var j=1;j<=(len-5)/4;j++){                               // 計算這筆訂單要印幾列
            $('table').append('<tr>'+
                                `<td id="td1${i}${j}">${outOrderArray[i].OutDate}</td>`+
                                `<td id="td2${i}${j}"">${outOrderArray[i].OrderID}</td>`+
                                `<td id="sel${i}${j}">${outOrderArray[i][`sel${j}`]}</td>`+
                                `<td id="ProdaName${i}${j}"></td>`+
                                `<td id="i4${i}${j}">${outOrderArray[i][`i4${j}`]}</td>`+
                                `<td id="i5${i}${j}">${outOrderArray[i][`i5${j}`]}</td>`+
                                `<td id="i6${i}${j}">${outOrderArray[i][`i6${j}`]}</td>`+
